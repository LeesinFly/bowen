...
if __name__ == "__main__":
    ...
    image_ids = open(os.path.join(VOCdevkit_path, "VOC2007/ImageSets/Main/test_gai.txt")).read().strip().split()
    ...
    if map_mode == 5:
        ...
        print("fps result.")
        image_fps = open(os.path.join(VOCdevkit_path, "VOC2007/ImageSets/Main/test_gai.txt")).read().strip().split()
        for image_id in tqdm(image_fps):
            image_path  = os.path.join(VOCdevkit_path, "VOC2007/JPEGImages/"+image_id+".jpg")
            image       = Image.open(image_path)
            if map_vis:
                image.save(os.path.join(map_out_path, "images-optional/" + image_id + ".jpg"))
            fps=yolo.get_FPS(image, test_interval=1000)
        print(fps)
        print("Get fps done.")