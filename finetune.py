...
def convert_to_training_format(data_point: Dict) -> Dict[str, str]:
    """
    将 fact_check_train.jsonl 中的每个条目转换为 instruction-input-output 格式，
    用于 LLM 微调。
    """

    # 提取原始字段
    label = data_point.get("label")
    question = data_point.get("query") or data_point.get("question", "")
    answer = data_point.get("response", "")
    evidence_list = data_point.get("evidence", [])

    # 构建 claim
    claim = answer  # 或者根据 question + response 构造 claim，例如：
    # claim = f"{question} The answer is {answer}"
    ...
    for e in evidence_list:
        if isinstance(e, list) and len(e) >= 3:
            ...
            relation = e[1]
            content = e[2]
            formatted_evidence.append(f"{title}: {relation} - {content}")
        elif isinstance(e, str):
            formatted_evidence.append(e)

    # 构建 input 字段（JSON string）
    input_str = json.dumps({
        "claim": claim,
        "evidence": formatted_evidence
    }, ensure_ascii=False)

    # 构建 output 字段（映射 FACTUAL/NON-FACTUAL → SUPPORTS/REFUTES）
    output_label = "SUPPORTS" if label == "FACTUAL" else "REFUTES"

    # 构建统一的 instruction（可自定义）
    instruction = (
        "I want you to act as a fallacy finder. You will be on the lookout for invalid arguments so "
        "you can call out any logical errors or inconsistencies that may be present in claim. Your job "
        "is to point out any fallacies, faulty reasoning, false assumptions, or incorrect conclusions that may be present in this claim. "
        "If you accept the claim, please output 'SUPPORTS'; if not, output 'REFUTES'."
    )
...
def add_args():
    ...
    parse.add_argument('--skip_conversion', action='store_true', help="If set, skips data conversion step in training. Useful when data is already converted.")
...
def train(options):
    ...
    print("正在加载 base model：", options.base_model)
    ...
    print("正在加载 tokenizer：", options.base_model)
